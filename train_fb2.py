...
if __name__ == "__main__":
    ...
    for step in range(max_steps):
        ...
        # if ((step > 0 and step % 250 == 0)) and (not use_compile):
            ...
            max_length = 32
            # TODO: look into why EOS tokens (id 1) are missing
            tokens = tokenizer("Once upon a time,", return_tensors="pt")['input_ids']

            tokens = tokens.type(torch.long)
            tokens = tokens.repeat(num_return_sequences, 1)
            xgen = tokens.to(device)

            goal_tokens = tokenizer("happily ever after.", return_tensors="pt")['input_ids']

            goal_tokens = goal_tokens.type(torch.long)
            goal_tokens = goal_tokens.repeat(num_return_sequences, 1)
